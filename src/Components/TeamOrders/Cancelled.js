...
function Cancelled() {
  ...
  const getOrders = async (pageNumberSelected = 1) => {
    ...
    Api()
      .post(`/team/get-orders/${canceled}/${pageNumberSelected}`, data)
      .then((response) => {
        setIsReady(true);
        ...
        newCounts['Canceled'] = response?.data.count
  ...
  const clientGetOrders = async (pageNumberSelected = 1) => {
    ...
    Api()
      .post(`/team/get-orders/${canceled}/${userAccountId}/${pageNumberSelected}`, data)
      .then((response) => {
        setIsReady(true);
        setOrders(response?.data?.data);
        setpageLength(response?.data?.pages);
        ...
        <div className="col-md-12 post-job-card">
          ...
          {!isReady ? (
            ...
            <div className="col-md-12 mt-4">
              ...
              <div className="d-flex justify-content-center align-item-center mt-5">
              <p>There are no canceled orders on the platform currently.</p>
            </div>
            </div>
          </div>
          ) : (
            <div className="row mb-4">
              {orders.map((order, index) => {
                return (
                  <Order order={order} key={index} route="/orders-canceled" />
                );
              })}
            </div>
          )}
