...
function NewOrders() {
  ...
  const [newOrders, setNewOrders] = useState("new");
  ...
  const getOrders = async (pageNumberSelected = 1) => {
    ...
    Api()
      .post(`/team/get-orders/${newOrders}/${pageNumberSelected}`, data)
      .then((response) => {
        setIsReady(true);
        ...
        newCounts["New"] = response?.data.count;
  ...
  const clientGetOrders = async (pageNumberSelected = 1) => {
    ...
    Api()
      .post(
        `/team/get-orders/${newOrders}/${userAccountId}/${pageNumberSelected}`,
        data
      )
      .then((response) => {
        setIsReady(true);
          ...
          {!isReady ? (
            ...
            <div className="row text-center">
              <div className="col-md-12 mt-4">
                <img
                  src="/images/Group 2678.svg"
                  alt="no job posted"
                  width="230"
                />
                <div className="d-flex justify-content-center align-item-center mt-5">
                  <p>
                    There are no hires currently. 
                    <span>
                 
                      <b
                        style={{ color: "#0A66C2" , paddingLeft: '3px', cursor: 'pointer'}}
                        onClick={() =>
            ...
            <div className="row mb-4">
              {orders.map((order, index) => {
                return <Order order={order} key={index} route="/team-orders" />;
              })}
            </div>
          )}
        </div>
