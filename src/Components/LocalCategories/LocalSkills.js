...
const LocalSkills = ({ categories, skills, changeData }) => {
  ...
  const handleDelete = () => {
    Api()
    .get(`/admin/industries/check-attachments/${deleteId}`)
    .then(
      (response)=>{
        ...
        >
          <CardContent>
            {categories?.map(
              (cate) =>
                cate.Industries &&
                cate.Industries.map((skillcategory) =>
                  skillcategory.parentId == skills ? (
                    ...
                    >
                      <Card key={skillcategory.id}
                        style={{ backgroundColor: "#2E405B26" }}
