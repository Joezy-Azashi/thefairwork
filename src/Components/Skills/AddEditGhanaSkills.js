...
import { Proficiency, SkillsType } from "../../Services/validation";
import { getCurrentUser, getUserType } from "../../Services/auth";
import Icon from "@mdi/react";
import { mdiClose } from "@mdi/js";

export default function AddEditGhanaSkills({
  editMode,
  skillToPass,
  handleClose,
  userIndustryType,
  ...
  const [skills, setSkills] = useState(
    localStorage.getItem("skillTypeData")
      ? JSON.parse(localStorage.getItem("skillTypeData"))
      : []
  );
  const [industrySkillId, setindustrySkillId] = useState()
  ...
  const [skillCertified, setSkillCertified] = useState(
    skillToPass?.certifySubIndustry
  );
  const [categoryCertified, setCategoryCertified] = useState(
    skillToPass?.certifyIndustry
  );
  ...
  const addSkill = async (e) => {
    const data = {
      ...
      industryId: industrySkillId?.id,
      categoryId: personSkill ? personSkill : null,
      ...
      name:
        ...
        userProfile?.UserProfile?.lastname === ""
          ...
          : userProfile?.UserProfile?.firstname +
            " " +
            userProfile?.UserProfile?.lastname,
      industryLink: JSON.parse(localStorage.getItem("categoryLink")),
      categotyLink: JSON.parse(localStorage.getItem("skillLink")),
    };


    if (personSkill === undefined || proficiency === undefined) {
      setAlert({
        open: true,
  ...
  const editSkill = async (e) => {
    const data = {
      id: skillData.id,
      userAccountId: accountId,
      categoryId: personSkill,
      type: userIndustryType,
      industryId: IndustryId,
      proficiency: proficiency,
      testScore: testScore ? testScore : null,
      certifyIndustry: localStorage.getItem("category")
        ? JSON.parse(localStorage.getItem("category"))
        : false,
        certifySubIndustry: localStorage.getItem("skill")
    ...
    Api()
      ...
      .then((response) => {
        ...
        const result = response?.data.find(
          ({ name }) => name === userIndustryType
        );
        setindustrySkillId(result)
