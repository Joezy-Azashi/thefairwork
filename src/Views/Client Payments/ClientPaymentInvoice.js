...
function ClientPaymentInvoice() {
  ...
  const clientGetOrders = async (pageNumberSelected = 1) => {
    ...
    Api()
      .post(`/users/get-client-paid-paymentInvoice/${pageNumberSelected}`, data)
      .then((response) => {
        setIsReady(true);
  ...
  const getPaymentResponse = async () => {
    Api()
      .get(`${api}/${`admin`}/${window.location.href.split("/")[4]}`)
      .then(async (response) => {
        openSnack();
        setAlert({
          open: true,
          message: `Payment for your order successfull`,
          severity: "success",
        });
        await clientGetOrders();
      })
      .catch(async (error) => {
        openSnack();
        setAlert({
  ...
  useEffect(() => {
    if (window.location.href.split("?")[0].split("/")[4] === "success") {
      getPaymentResponse();
    } else {
      clientGetOrders();
    }
    ...
    <div>
      <div className="row">
        ...
        <div className="col-md-12 paymentOrderDesktop">
          <PaymentHeader
            titlePage="Completed Payments"
            secondColumn="Issue Date"
            thirdColumn="Payment Date"
          />
            ...
            >
              <div className="row text-center mt-5 mb-5">
                <div className="col-md-12">
                  <img
                    src="/images/Group 2403.png"
                    alt="no payments to be made"
                  />
                  <p className="mt-4">There are no invoice currently </p>
                </div>
              </div>
            </Card>
          ) : (
              ...
              {paidInvoices.map((invoice, index) => {
                return (
                  <PaymentOrder
                    ...
                    paymentDate={moment(`${invoice?.invoiceDate}`).format(
                      "Do MMMM YYYY"
                    )}
                    paymentDueDate={moment(`${invoice?.dueDate}`).format(
                      "Do MMMM YYYY"
                    )}
                    action="View Invoice"
                    key={index}
                    ...
                    {paidInvoices.map((invoice, index) => {
                      return (
                        <TableRow key={index}>
                          ...
                          {moment(`${invoice?.invoiceDate}`).format("Do MMMM YYYY") === "Invalid date" ? `Not Provided` : moment(`${invoice?.invoiceDate}`).format("Do MMMM YYYY") } 
                          </TableCell>
                          <TableCell align="left">
...
export default ClientPaymentInvoice;
